...
# Load keys (will raise errors if not set in .env)
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY')
GITHUB_TOKEN = os.getenv('GITHUB_TOKEN')
DISCORD_BOT_TOKEN = os.getenv('DISCORD_BOT_TOKEN')

if not all([GEMINI_API_KEY, GITHUB_TOKEN, DISCORD_BOT_TOKEN]):
    raise ValueError("Missing API keys in .env file. Please set GEMINI_API_KEY, GITHUB_TOKEN, and DISCORD_BOT_TOKEN.")
...
class GamifiedGitHubDiscordBot(commands.Bot):
    ...
    def graphql_query(self, query, variables=None):
        ...
        try:
            headers, response = self.github._Github__requester.requestJsonAndCheck(
                "POST", "/graphql", input=post_data
            )
    ...
    def get_user_total_activity(self, gh_user):
        ...
        """
        # Issues count
        issues_vars = {"searchQuery": f"repo:{owner}/{name} is:issue author:{gh_user}"}
        data_issues = self.graphql_query(search_query, issues_vars)
        issues = data_issues['search']['issueCount']
        
        # PRs count
        prs_vars = {"searchQuery": f"repo:{owner}/{name} is:pr author:{gh_user}"}
        data_prs = self.graphql_query(search_query, prs_vars)
        prs = data_prs['search']['issueCount']
        
        # Commits (REST API, exact)
    ...
    def add_commands(self):
        ...
        async def link_github(ctx, github_username: str):
            ...
            await ctx.send(f"Linked @{github_username}! **{points} points** awarded for {commits} commits/{issues} issues/{prs} prs!")
        ...
        async def my_challenge(ctx):
            ...
            activity = await self.get_user_activity(user_info['github_user'])
    ...
    async def poll_github_once(self):
        ...
        for disc_id, data in list(self.user_data.items()):
            ...
            try:
                ...
                # Delta commits
                commits_d = await self.loop.run_in_executor(
                    None, lambda: self.repo.get_commits(author=gh_user, since=last_check).totalCount
                )
                
                # Delta issues (GraphQL)
                issues_q = f"repo:{owner}/{name} is:issue author:{gh_user} created:>={date_str}"
                data_issues = await self.loop.run_in_executor(
                    None, self.graphql_query, search_query, {"searchQuery": issues_q}
                )
                issues_d = data_issues['search']['issueCount']
                
                # Delta PRs (GraphQL)
                prs_q = f"repo:{owner}/{name} is:pr author:{gh_user} created:>={date_str}"
                data_prs = await self.loop.run_in_executor(
                    None, self.graphql_query, search_query, {"searchQuery": prs_q}
                )
                prs_d = data_prs['search']['issueCount']
                
                new_points = commits_d * 10 + issues_d * 20 + prs_d * 30
                if new_points > 0:
                    data['points'] += new_points
                    self.update_badges(data)
                    logger.info(f"{gh_user}: +{new_points} pts (delta)")
            except Exception as e:
                logger.error(f"Poll error for {gh_user}: {e}")
